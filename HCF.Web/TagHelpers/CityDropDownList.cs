...
{
    [HtmlTargetElement("citydropdownlist")]
    public class CityDropDownList : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly ISiteService _constructionService;

        public CityDropDownList(IHtmlHelper helper, ISiteService constructionService)
        {
            _html = helper;
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var cities = _constructionService.GetCities(StateId).Where(x => x.IsActive).ToList();
           
            foreach (var item in cities)
            {
                var selItem = new SelectListItem { Text = item.CityName,
