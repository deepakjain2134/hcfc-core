...
{
    [HtmlTargetElement("riskrypedropdown")]
    public class RiskTypeDropDown : TagHelper
    {
        private readonly IHtmlHelper _html;

        public RiskTypeDropDown(IHtmlHelper helper)
        {
            _html = helper;
        }
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var lists = from BDO.Enums.RiskScore e in Enum.GetValues(typeof(BDO.Enums.RiskScore))
                        select new
                        {
                            Value = (int)e,
