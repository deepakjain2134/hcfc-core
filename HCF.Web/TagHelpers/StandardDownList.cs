...
{
    [HtmlTargetElement("standarddownlist")]
    public class StandardDownList : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly IStandardService _service;

        public StandardDownList(IHtmlHelper helper, IStandardService service)
        {
            _html = helper;
            _service = service;
        }
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var lists = _service.GetStandards().Where(x => x.IsActive).ToList();
