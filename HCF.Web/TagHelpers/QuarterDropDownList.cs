...
{
    [HtmlTargetElement("quarterdropdownlist")]
    public class QuarterDropDownList : TagHelper
    {
        private readonly IHtmlHelper _html;

        public QuarterDropDownList(IHtmlHelper helper)
        {
            _html = helper;
        }
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var approvalStatus = from BDO.Enums.QuarterNo e in Enum.GetValues(typeof(BDO.Enums.QuarterNo))
                                 select new
                                 {
                                     Value = (int)e,
                                     Text = string.Format("{0}{1} ({2})", "Q", (int)e, e.GetDescription().ToString())
                                 };
