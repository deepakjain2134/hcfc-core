...
{
    public class RoundCategoryDropDown : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly IRoundsService _service;

        public RoundCategoryDropDown(IHtmlHelper helper, IRoundsService service)
        {
            _html = helper;
            _service = service;
        }
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var roundsCateg = _service.GetRoundCategories_Data().ToList(); //.Where(x => x.IsActive).ToList();
