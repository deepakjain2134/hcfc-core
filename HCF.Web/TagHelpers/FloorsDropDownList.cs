...
{
    [HtmlTargetElement("floorsdropdownlist")]
    public class FloorsDropDownList : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly IFloorService _service;

        public FloorsDropDownList(IHtmlHelper helper, IFloorService service)
        {
            _html = helper;
            _service = service;
        }
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var floors = _service.GetFloors().Where(x => x.BuildingId == BuildingId 
            && x.IsActive).ToList();
