...
{
    [HtmlTargetElement("icraprojectdropdown")]
    public class ICRAProjectDropDown : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly ITIcraProjectService _constructionService;

        public ICRAProjectDropDown(IHtmlHelper helper, ITIcraProjectService constructionService)
        {
            _html = helper;
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var projects = _constructionService.GetAllActiveTIcraProject(HideInactiveValue).ToList();

            if (!HideInactiveValue)
            {
                projects = projects.Where(x => x.Status == true || x.ProjectId ==Convert.ToInt32(
