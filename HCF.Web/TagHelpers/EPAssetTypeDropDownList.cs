...
{
    [HtmlTargetElement("epassettypedropdownList")]
    public class EPAssetTypeDropDownList : TagHelper
    {
        private readonly IHtmlHelper _html;
        private readonly IAssetsService  _assetsService;

        public EPAssetTypeDropDownList(IHtmlHelper helper, IAssetsService service)
        {
            _html = helper;
        ...
        private List<SelectListItem> BindItems()
        {
            ...
            var lists = _assetsService.GetAssetsByType(UserSession.CurrentUser.UserId, 0).Where(y => y.Assets.Any(x => x.Count > 0 && x.StandardEps.Any())).ToList();
