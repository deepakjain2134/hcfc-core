...
namespace HCF.Web.Base
{
    public class UISession
    {
        ...
        private static List<UserNavigation> NavigationSession
        {
            get
            {
        ...
        private static string GetReffUrl()
        {
            var request = ServiceLocator.ServiceProvider.GetService<IHttpContextAccessor>().HttpContext.Request;
        ...
        public static void AddCurrentPage(string screenName, int pageIndex, string displayName, bool isRoot = false)
        {
        ...
        public static UserNavigation AddToList(string screenName, int pageIndex, string displayName)
        {
            ...
            if (!pageUrl.Contains("Isback"))
            {
                ...
                if (loalNavigationSession.ToList().Count > 0)
                    _pageIndex = loalNavigationSession.Max(x => x.PageIndex) + 1;
                ...
                userNaviagtion.UserId = UserSession.CurrentUser.UserId;
        ...
        public static string GetBackPage(string screenName)
        {
        ...
        public static string GetBackPagebyPageUrl(string pageUrl, string screenName)
        {
        ...
        public static List<UserNavigation> GetAll()
        {
        ...
        private static List<UserNavigation> BreadCrumbSession
        {
            get
            {
        ...
        public static UserNavigation AddToBreadCrumbList(string screenName, int pageIndex, string displayName)
        {

            var localBreadCrumbSession = BreadCrumbSession;
            //if (pageIndex == 0)
            //    localBreadCrumbSession = new List<UserNavigation>();
            ...
            int _pageIndex = 1;
            if (localBreadCrumbSession.ToList().Count > 0)
                _pageIndex = localBreadCrumbSession.Max(x => x.PageIndex) + 1;
        ...
        {
            List<UserNavigation> localBreadCrumbSession = new List<UserNavigation>();
            var sessionObj = ServiceLocator.ServiceProvider.GetService<IHCFSession>();
            sessionObj.Set(C_SESSION_KEY_BREADCRUMB, localBreadCrumbSession);
            //BreadCrumbSession?.Clear();
        }

        public static string GetBackBreadCrumb(int pageIndex, string screenName)
        {
        ...
        public static List<UserNavigation> GetBreadCrumbs()
        {
            return BreadCrumbSession.ToList();
        }

        public static string GetBackPageInspection(string screenName)
        {
        ...
        public static bool IskeyExist(string screenName)
        {
            return BreadCrumbSession.Any(x => x.Screen == screenName);
        }


        public static string GetPreviousPage(string screenName)
        {
            var redirectPage = NavigationSession.Where(x => x.Screen == screenName).OrderByDescending(x => x.CreatedDate).FirstOrDefault();
            if (redirectPage != null)
            {
